...
nc82decision="Signal_amount";//"Signal_amount";//"Signal_amount","ch1","ch2","ch3","ch4"

// Arguments

argstr=0;
inputfile="JRC_SS103489-20241101_44_F5-m_20x_VNC.v3draw.zip";
...
//for M2 air
//argstr="/Users/otsunah/test/VNC_aligner_local_ver/output3/,"+inputfile+",/Users/otsunah/test/VNC_aligner_local_ver/Template/,/Users/otsunah/test/VNC_aligner_local_ver/sample/"+inputfile+",sr,f,/test/VNC_aligner/Sample/ConsolidatedLabel.v3dpbd,6,true,Signal_amount";//for test
...
if(args.length==10){
	...
	temptype=args[8];//"f" or "m"
	PathConsolidatedLabel=args[9];// full file path for ConsolidatedLabel.v3dpbd
	numCPU=args[10];
	ShapeAnalysis=args[11];//true or false
...
function God(savedir, noext,origi,Batch,myDir0,chanspec,temptype,AdvanceDepth,LateralDir,PathConsolidatedLabel,numCPU,path,ShapeAnalysis,UPsideDown){
	...
	if(channels>1){
		...
		if(donotOperate==0){
			...
			if(args.length==12){
				run("Properties...", "channels=1 slices="+nSlices+" frames=1 unit=pixels pixel_width="+RESX+" pixel_height="+RESY+" voxel_depth="+RESZ+"");//setting property, voxel size 1,1,1 for later translation.
			}
				...
				if(donotOperate==0){//open from directory, not from Image
					...
					if(MIPsave==1){
						...
						if(AdvanceDepth){
							...
							startWidth=round(defaultWidth/2);
							
							print("sampWidth; "+sampWidth+"   sampHeight; "+sampHeight+"   nImages;"+nImages()+"   defaultWidth; "+defaultWidth+"  depthVXsmall; "+depthVXsmall);
							...
							print("");
							print("BestWidth; "+BestWidth+"  BestWithOriginalVal; "+BestWithOriginalVal+"  maxrotation; "+maxrotation+"  FlipPosi;"+FlipPosi+"  maxOBJ; "+maxOBJ);
...
function Helongate (ElongArray,sampHeight,heightSizeRatio,lateralNC82stack,tempimg,lateralNC82stackST){
	...
	tryN=0;
	maxW=0; maxOBJ=0; nonmaxOBJtime=0; MinusRot=15; PlusRot=15; maxrotation=0; MaxShiftABS=20; NextRotation=0;
