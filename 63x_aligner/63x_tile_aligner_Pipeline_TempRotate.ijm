...
testArg = "/test/63x_align/,JRC_SS30452_20170120_22_H2.v3dpbd,/test/63x_align/Hemi_Brain_OP/JRC_SS30452_20170120_22_H2.v3dpbd,/test/63x_align/Template/,0.188,0.38,11,63x,f";
...
if(FromDir==1){
	...
	dotIndexLSM = -1;
	dotIndexLSM = lastIndexOf(filename, ".lsm");
	
	dotIndexzip = -1;
	dotIndexzip = lastIndexOf(filename, ".zip");
	
	dotIndexv3d = -1;
	dotIndexv3d = lastIndexOf(filename, ".v3dpbd");
	if(dotIndexv3d==-1)
	dotIndexv3d = lastIndexOf(filename, ".v3draw");
	
	dotIndextif = lastIndexOf(filename, ".tif");
	
	dotIndexnrrd = lastIndexOf(filename, ".nrrd");
	
	dotIndexH5j = lastIndexOf(filename, ".h5j");
	
	dotIndexFolder = -1;
	dotIndexFolder = lastIndexOf(filename, "/");
	...
	noext2=0;
	setBatchMode(true);
	if(dotIndexLSM>-1 || dotIndexzip>0 || dotIndexv3d>-1 || dotIndextif>-1 || dotIndexnrrd>-1 || dotIndexH5j>-1){
		...
		DupIndex=lastIndexOf(SampleTitle, "_201");
		
		if(dotIndexv3d>-1 || dotIndexzip!=-1 || dotIndextif!=-1){
			DupIndex=lastIndexOf(SampleTitle, ".");
		}
		...
		if(dotIndexH5j!=-1 || dotIndexv3d!=-1){
			print("Channels; "+channels+"  slices; "+nSlices()/channels+"  voxel size changed");
			run("Properties...", "channels="+channels+" slices="+nSlices()/channels+" frames=1 unit=microns pixel_width=0.1882680 pixel_height=0.1882680 voxel_depth=0.38");
		...
		if(channels>1){
			...
			if(dotIndexH5j==-1 && dotIndexv3d==-1){
				for(ichannel=1; ichannel<=channels; ichannel++){
					...
					getStatistics(area, meanArray[ichannel-1], min, max, std, histogram);
					close();
					if(maxMean<meanArray[ichannel-1]){
						maxMean=meanArray[ichannel-1];
						nc82Channel=ichannel;
					}
		...
		if(isOpen(nc82Ori) && dotIndexnrrd==-1){
			selectImage(nc82Ori);
			close();
		}
		startNrrdNo=0;
